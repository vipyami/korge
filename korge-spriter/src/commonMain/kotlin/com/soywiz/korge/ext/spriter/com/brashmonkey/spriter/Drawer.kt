 ...
 */
	...
	fun drawBone(bone: Bone, size: Dimension) {
		...
		val xx = bone.position.x + cos(Angle.toRadians(bone._angle)) * size.height
		val yy = bone.position.y + sin(Angle.toRadians(bone._angle)) * size.height
		val x2 = cos(Angle.toRadians((bone._angle + 90))) * halfHeight * bone.scale.y
		val y2 = sin(Angle.toRadians((bone._angle + 90))) * halfHeight * bone.scale.y

		val targetX =
			bone.position.x + cos(Angle.toRadians(bone._angle)) * size.width * bone.scale.x
		val targetY =
			bone.position.y + sin(Angle.toRadians(bone._angle)) * size.width * bone.scale.x
	...
	fun drawPoints(player: Player, it: Iterator<Object> = player.objectIterator()) {
		while (it.hasNext()) {
			...
			if (player.getObjectInfoFor(point).type == ObjectType.Point) {
				val x = point.position.x + (cos(Angle.toRadians(point._angle)) * pointRadius)
				val y = point.position.y + (sin(Angle.toRadians(point._angle)) * pointRadius)
