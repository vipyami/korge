...
class CCDResolver(player: Player) : IKResolver(player) {
	...
	public override fun resolve(x: Float, y: Float, chainLength: Int, effector: BoneRef) {
		...
		var xx = unmappedffector.position.x + cos(Angle.toRadians(unmappedffector._angle)) * width
		var yy = unmappedffector.position.y + sin(Angle.toRadians(unmappedffector._angle)) * width
		...
		while (i < chainLength && parentRef != null) {
			...
			if (parentRef != null && i < chainLength - 1) {
				...
				parent = null
			player.unmapObjects(null)
			xx = unmappedffector.position.x + cos(Angle.toRadians(unmappedffector._angle)) * width
			yy = unmappedffector.position.y + sin(Angle.toRadians(unmappedffector._angle)) * width
			i++
		}
	}
